...
| Field | Notes |
|---|---|
| `id` | **Permanent. Never renumber.** See below |
...
- **`id` is permanent and the array order does not set it. Enforced** - required,
  integer, non-negative and unique. It is what `/country/#event-N` addresses, so
  renumbering silently repoints every shared link at a different event.

  It used to be `enumerate(events)` in `build.py`, which meant inserting one
  event in the middle repointed every link after it. That blocked a real fix
  twice before the ids were frozen into the data on 26/08/2026. **A new event
  takes `max(existing) + 1` wherever it sits in the order**, and moving an event
  means changing its `sort_year` and its array position, never its id. The array
  should still be sorted by `sort_year`; the ids simply will not be, and that is
  correct.
...
A title must be a real title, written as a title. Taiwan's were generated by
splitting the description on `.`, which cut them mid-sentence and mid-number;
95 were rewritten on 20/08/2026 and a further 19 on 26/08/2026, along with 57
descriptions that had a source's sub-heading welded onto the front of them. Five
validator rules now stop that particular shape coming back. Do not copy the
pattern that caused it: derive nothing from splitting prose.
